...
from search_spaces.MobileNetV3.modules.dynamic_op import (
    DynamicSeparableConv2d,
    DynamicInSeparableConv2dMixture,
    DynamicInConv2dMixture,
    DynamicOutConv2dMixture,
    DynamicConv2d,
    DynamicBatchNorm2d,
    DynamicBatchNorm2dMixture,
    DynamicSE,
    DynamicSEMixture,
    DynamicGroupNorm,
    DynamicGroupNormMixture
)
...
def copy_bn(target_bn, src_bn):
    ...
    if type(src_bn) in [nn.BatchNorm1d, nn.BatchNorm2d]:
        target_bn.running_mean.data.copy_(
...
class DynamicMBConvLayerMixture(MyModule):
    def __init__(
        self,
        in_channel_list,
        out_channel_list,
        ...
        super(DynamicMBConvLayerMixture, self).__init__()

        self.in_channel_list = in_channel_list
        self.out_channel_list = out_channel_list
        self.mid_channel_list_options = [round(max(self.in_channel_list) * e, MyNetwork.CHANNEL_DIVISIBLE,) for e in expand_ratio_list]
        self.kernel_size_list = val2list(kernel_size_list)
        self.expand_ratio_list = val2list(expand_ratio_list)
        self.mixop = mixop
        self.stride = stride
                ...
                OrderedDict(
                    [
                        (
                            "conv",
                            DynamicOutConv2dMixture(
                                max(self.in_channel_list), self.mid_channel_list_options
                            ),
                        ),
                        ("bn", DynamicBatchNorm2dMixture(
                            self.mid_channel_list_options)),
                        ("act", build_activation(self.act_func)),
                    ]
                )
            )
            self.inverted_bottleneck_0_op_list = self.get_entangle_ops(self.inverted_bottleneck[0], self.expand_ratio_list, "expand_ratio")
            self.inverted_bottleneck_1_op_list = self.get_entangle_ops(self.inverted_bottleneck[1], self.expand_ratio_list, "expand_ratio")
            ...
            OrderedDict(
                [
                    (
                        "conv",
                        DynamicInSeparableConv2dMixture(
                            self.mid_channel_list_options, self.kernel_size_list, self.stride
                        ),
                    ),
                    ("bn", DynamicBatchNorm2dMixture(
                        self.mid_channel_list_options)),
        ...
        self.depth_conv_0_op_list = self.get_entangle_ops_combi(self.depth_conv[0], self.mid_channel_list_options, self.kernel_size_list, "expand_ratio_kernel_size")
        self.depth_conv_1_op_list = self.get_entangle_ops(self.depth_conv[1], self.expand_ratio_list, "expand_ratio")
        if self.use_se:
            self.depth_conv.add_module("se", DynamicSEMixture(
                self.mid_channel_list_options))
            self.depth_conv_3_op_list = self.get_entangle_ops(self.depth_conv[3], self.expand_ratio_list, "expand_ratio")
            ...
            OrderedDict(
                [
                    (
                        "conv",
                        DynamicInConv2dMixture(
                            self.mid_channel_list_options, max(self.out_channel_list)),
                    ),
                    ("bn", nn.BatchNorm2d(max(self.out_channel_list))),
                ]
            )
        )
        self.point_linear_0_op_list = self.get_entangle_ops(self.point_linear[0], self.expand_ratio_list,"expand_ratio")

        self.active_kernel_size = max(self.kernel_size_list)
        self.active_expand_ratio = max(self.expand_ratio_list)
        self.active_out_channel = max(self.out_channel_list)

    def forward(self, x, expand_ratio_weights, kernel_size_weights, use_argmax=False):
        if self.inverted_bottleneck is not None:
            #x = self.inverted_bottleneck[0](x, expand_ratio_weights, use_argmax=use_argmax)
            x = self.mixop.forward(x, expand_ratio_weights, self.inverted_bottleneck_0_op_list)
            #x = self.inverted_bottleneck[1](x, expand_ratio_weights, use_argmax=use_argmax)
            x = self.mixop.forward(x, expand_ratio_weights, self.inverted_bottleneck_1_op_list)
            x = self.inverted_bottleneck[2](x)

        #x = self.depth_conv[0](x, expand_ratio_weights, kernel_size_weights, use_argmax=use_argmax)
        x = self.mixop.forward(x, [expand_ratio_weights,kernel_size_weights], self.depth_conv_0_op_list, combi=True)
        #x = self.depth_conv[1](x, expand_ratio_weights, use_argmax=use_argmax)
        x = self.mixop.forward(x, expand_ratio_weights, self.depth_conv_1_op_list)
        x = self.depth_conv[2](x)
        if self.use_se:
            #x = self.depth_conv[3](x, expand_ratio_weights, use_argmax=use_argmax)
            x = self.mixop.forward(x, expand_ratio_weights, self.depth_conv_3_op_list)
        #x = self.point_linear[0](x, expand_ratio_weights, use_argmax=use_argmax)
        x = self.mixop.forward(x, expand_ratio_weights, self.point_linear_0_op_list)
        x = self.point_linear[1].to(x.device)(x)
        return x
...
# test DynamicMBConvLayerMixture
