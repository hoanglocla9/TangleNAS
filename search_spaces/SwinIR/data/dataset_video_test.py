...
class VideoRecurrentTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        for subfolder_lq, subfolder_gt in zip(subfolders_lq, subfolders_gt):
            ...
            img_paths_gt = sorted(
                list(utils_video.scandir(subfolder_gt, full_path=True)))
            ...
            if self.cache_data:
                ...
                self.imgs_lq[subfolder_name] = utils_video.read_img_seq(
                    img_paths_lq)
                self.imgs_gt[subfolder_name] = utils_video.read_img_seq(
                    img_paths_gt)
            else:
        ...
        self.sigma = opt[
            'sigma'] / 255. if 'sigma' in opt else 0  # for non-blind video denoising
...
class VFI_DAVIS(data.Dataset):
    ...
    def __init__(self, data_root, ext="png"):
        ...
        for label_id in os.listdir(self.data_root):
            ctg_imgs_ = sorted(
                os.listdir(os.path.join(self.data_root, label_id)))
            ctg_imgs_ = [
                os.path.join(self.data_root, label_id, img_id)
                for img_id in ctg_imgs_
            ]
            for start_idx in range(0, len(ctg_imgs_) - 6, 2):
                add_files = ctg_imgs_[start_idx:start_idx + 7:2]
                add_files = add_files[:2] + [ctg_imgs_[start_idx + 3]
                                             ] + add_files[2:]
...
class VFI_Vid4(data.Dataset):
    ...
    def __init__(self, data_root, ext="png"):
        ...
        for label_id in os.listdir(self.data_root):
            ...
            for start_idx in range(0, len(ctg_imgs_) - 6, 2):
                add_files = ctg_imgs_[start_idx:start_idx + 7:2]
                self.data_info['lq_path'].append(
                    [os.path.basename(path) for path in add_files])
                self.data_info['gt_path'].append(
                    os.path.basename(ctg_imgs_[start_idx + 3]))
