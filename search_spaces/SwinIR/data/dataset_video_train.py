...
class VideoRecurrentTrainDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        with open(opt['meta_info_file'], 'r') as fin:
            for line in fin:
                ...
                total_num_frames.extend(
                    [int(frame_num) for i in range(int(frame_num))])
                start_frames.extend(
                    [int(start_frame) for i in range(int(frame_num))])
    ...
    def __getitem__(self, index):
        if self.file_client is None:
            self.file_client = utils_video.FileClient(
        ...
        if start_frame_idx > endmost_start_frame_idx:
            start_frame_idx = random.randint(start_frames,
                                             endmost_start_frame_idx)
        ...
        img_results = utils_video.augment(img_lqs, self.opt['use_hflip'],
                                          self.opt['use_rot'])
...
class VideoRecurrentTrainNonblindDenoisingDataset(VideoRecurrentTrainDataset):
    ...
    def __getitem__(self, index):
        ...
        # randomly crop
        img_gts, _ = utils_video.paired_random_crop(img_gts, img_gts,
                                                    self.gt_size, 1,
                                                    img_gt_path)

        # augmentation - flip, rotate
        img_gts = utils_video.augment(img_gts, self.opt['use_hflip'],
                                      self.opt['use_rot'])
        ...
        noise_level = torch.empty(
            (1, 1, 1, 1)).uniform_(self.sigma_min, self.sigma_max)
...
class VideoRecurrentTrainVimeoVFIDataset(VideoRecurrentTrainVimeoDataset):
    ...
    def __init__(self, opt):
        ...
        if self.color_jitter:
            self.transforms_color_jitter = transforms.ColorJitter(
                0.05, 0.05, 0.05, 0.05)
