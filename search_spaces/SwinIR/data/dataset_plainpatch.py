...
class DatasetPlainPatch(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.num_patches_per_image = opt['num_patches_per_image'] if opt[
            'num_patches_per_image'] else 40
        ...
        if self.paths_L and self.paths_H:
            assert len(self.paths_L) == len(
                self.paths_H
            ), 'H and L datasets have different number of images - {}, {}.'.format(
        ...
        self.H_data = np.zeros([
            self.total_patches, self.path_size, self.path_size, self.n_channels
        ],
                               dtype=np.uint8)
        self.L_data = np.zeros([
            self.total_patches, self.path_size, self.path_size, self.n_channels
        ],
                               dtype=np.uint8)
    ...
    def update_data(self):
        ...
        # ------------------------------------
        """
        self.index_sampled = random.sample(range(0, len(self.paths_H), 1),
                                           self.num_sampled)
        n_count = 0
        ...
        print(
            'Training data updated! Total number of patches is:  %5.2f X %5.2f = %5.2f\n'
