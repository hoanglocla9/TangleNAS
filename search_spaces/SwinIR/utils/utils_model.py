...
def find_last_checkpoint(save_dir, net_type='G', pretrained_path=None):
    ...
    if file_list:
        ...
        init_path = os.path.join(save_dir,
                                 '{}_{}.pth'.format(init_iter, net_type))
    else:
        init_iter = 0
...
def test_split_fn(model, L, refield=32, min_size=256, sf=1, modulo=1):
    ...
    if h * w <= min_size**2:
        L = torch.nn.ReplicationPad2d(
            (0, int(np.ceil(w / modulo) * modulo - w), 0,
             int(np.ceil(h / modulo) * modulo - h)))(L)
        E = model(L)
        E = E[..., :h * sf, :w * sf]
    else:
        ...
        Ls = [
            L[..., top, left], L[..., top, right], L[..., bottom, left],
            L[..., bottom, right]
        ]
        ...
        if h * w <= 4 * (min_size**2):
            ...
            Es = [
                test_split_fn(model,
                              ...
                              min_size=min_size,
                              sf=sf,
                              modulo=modulo) for i in range(4)
            ]

        b, c = Es[0].size()[:2]
        E = torch.zeros(b, c, sf * h, sf * w).type_as(L)

        E[..., :h // 2 * sf, :w // 2 * sf] = Es[0][..., :h // 2 * sf, :w // 2 *
                                                   sf]
        E[..., :h // 2 * sf, w // 2 * sf:w * sf] = Es[1][..., :h // 2 * sf,
                                                         (-w + w // 2) * sf:]
        E[..., h // 2 * sf:h * sf, :w // 2 * sf] = Es[2][..., (-h + h // 2) *
                                                         sf:, :w // 2 * sf]
        E[..., h // 2 * sf:h * sf,
          w // 2 * sf:w * sf] = Es[3][..., (-h + h // 2) * sf:,
                                      (-w + w // 2) * sf:]
    return E
