...
def get_uperleft_denominator(img, kernel):
    ...
    upperleft = np.expand_dims(np.conj(V), axis=2) * np.fft.fft2(img,
                                                                 axes=[0, 1])
...
def p2o(psf, shape):
    ...
    otf[...,
        1][torch.abs(otf[...,
                         1]) < n_ops * 2.22e-16] = torch.tensor(0).type_as(psf)
    return otf
...
def solve_min_laplacian(boundary_image):
    ...
    f_bp = np.zeros((H, W))
    f_bp[np.ix_(
        j, k)] = -4 * boundary_image[np.ix_(j, k)] + boundary_image[np.ix_(
            j,
            k + 1)] + boundary_image[np.ix_(j, k - 1)] + boundary_image[np.ix_(
                j - 1, k)] + boundary_image[np.ix_(j + 1, k)]
    ...
    denom = (2 * np.cos(np.pi * x / (W - 1)) - 2) + (2 * np.cos(np.pi * y /
                                                                (H - 1)) - 2)
    ...
    if tt.shape[1] == 1:
        img_tt = np.transpose(
            fftpack.idst(np.transpose(tt) * 2, type=1) / (2 *
                                                          (tt.shape[0] + 1)))
    else:
        img_tt = np.transpose(
            fftpack.idst(np.transpose(tt) * 2, type=1, axis=0) /
            (2 * (tt.shape[1] + 1)))
    del (tt)
...
def fspecial_disk(radius):
    ...
    [x, y] = np.meshgrid(np.arange(-crad, crad + 1),
                         np.arange(-crad, crad + 1))
...
def blurkernel_synthesis(h=37, w=None):
    ...
    if np.random.randint(0, 4) == 1:
        k = cv2.resize(
            k, (random.randint(x1, 5 * x1), random.randint(x2, 5 * x2)),
            interpolation=cv2.INTER_LINEAR)
        y1, y2 = k.shape
        k = k[(y1 - x1) // 2:(y1 - x1) // 2 + x1,
              (y2 - x2) // 2:(y2 - x2) // 2 + x2]
...
def rot3D(x, r):
    Rx = array([[1, 0, 0], [0, cos(r[0]), -sin(r[0])],
                [0, sin(r[0]), cos(r[0])]])
    Ry = array([[cos(r[1]), 0, sin(r[1])], [0, 1, 0],
                [-sin(r[1]), 0, cos(r[1])]])
    Rz = array([[cos(r[2]), -sin(r[2]), 0], [sin(r[2]),
                                             cos(r[2]), 0], [0, 0, 1]])
    R = Rz @ Ry @ Rx
    x = R @ x
    return x
