...
def cal_pca_matrix(path='PCA_matrix.mat',
                   ksize=15,
                   l_max=12.0,
                   dim_pca=15,
                   num_samples=500):
    ...
    for i in range(num_samples):
        ...
        k = anisotropic_Gaussian(ksize=ksize,
                                 theta=theta[0],
                                 l1=l1[0],
                                 l2=l2[0])
...
def gen_kernel(k_size=np.array([25, 25]),
               scale_factor=np.array([4, 4]),
               min_var=0.6,
               max_var=12.,
               noise_level=0):
...
def upsample(x, sf=3, center=False):
    ...
    z = torch.zeros(
        (x.shape[0], x.shape[1], x.shape[2] * sf, x.shape[3] * sf)).type_as(x)
...
def dim_pad_circular(input, padding, dimension):
    ...
    input = torch.cat([
        input[[slice(None)] *
              (dimension - 1) + [slice(-2 * padding, -padding)]], input
    ],
                      dim=dimension - 1)
    return input
...
def imfilter(x, k):
    ...
    '''
    x = pad_circular(x,
                     padding=((k.shape[-2] - 1) // 2, (k.shape[-1] - 1) // 2))
...
def blockproc(im, blocksize, fun):
    xblocks = np.split(im,
                       range(blocksize[0], im.shape[0], blocksize[0]),
                       axis=0)
    xblocks_proc = []
    for xb in xblocks:
        yblocks = np.split(xb,
                           range(blocksize[1], im.shape[1], blocksize[1]),
                           axis=1)
        yblocks_proc = []
        for yb in yblocks:
            yb_proc = fun(yb)
...
if __name__ == '__main__':
    ...
    k = shifted_anisotropic_Gaussian(k_size=np.array([15, 15]),
                                     scale_factor=np.array([4, 4]),
                                     min_var=0.8,
                                     max_var=10.8,
                                     noise_level=0.0)
