...
class ModelPlain(ModelBase):
    ...
    def __init__(self, opt):
        ...
        self.config_best = self.netG.get_best_config()
    ...
    def load_optimizers(self):
        ...
        if load_path_optimizerG is not None and self.opt_train[
                'G_optimizer_reuse']:
    ...
    def arch_parameters(self):
        return self.netG.module.arch_parameters()

    def define_optimizer(self):
        G_optim_params = []
        for k, v in self.netG.named_parameters():
            if v.requires_grad and ("alpha" not in k):
    ...
    def get_best_config(self):
        self.netG.module.get_best_config()
        ...
        if self.opt_train['G_scheduler_type'] == 'MultiStepLR':
            self.schedulers.append(
                lr_scheduler.MultiStepLR(
                    self.G_optimizer, self.opt_train['G_scheduler_milestones'],
                    self.opt_train['G_scheduler_gamma']))
        elif self.opt_train[
                'G_scheduler_type'] == 'CosineAnnealingWarmRestarts':
            self.schedulers.append(
                lr_scheduler.CosineAnnealingWarmRestarts(
                    self.G_optimizer, self.opt_train['G_scheduler_periods'],
    ...
    # feed L to netG
    # ----------------------------------------
    def netG_forward(self, tau):
        self.E, self.params = self.netG(self.L, self.arch_params, tau)

    def optimize_arch_params(self, tau, arch_params=None):
        self.arch_params = arch_params
        self.netG_forward(tau)
        G_loss = self.G_lossfn_weight * self.G_lossfn(
            self.E, self.H)  #+ self.params_penalty_w*self.params
        return G_loss

    # ----------------------------------------
    # update parameters and get loss
    # ----------------------------------------
    def optimize_parameters(self, current_step, tau, arch_params=None):
    ...
    # test / inference
    # ----------------------------------------
    def test(self, tau_curr):
        self.netG.eval()
        with torch.no_grad():
            self.netG_forward(tau_curr)
        self.netG.train()
