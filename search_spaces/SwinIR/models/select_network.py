...
def define_G(opt):
    ...
    # ----------------------------------------
        ...
        )  #["params"])#[param_key_g] if param_key_g in pretrained_model.keys() else pretrained_model,strict=True)

    elif net_type == 'swinir_search':
        from models.network_swinir_one_shot import SwinIR as net
        netG = net(opt_net["os_optim"],
                   opt_net["config"],
