...
class Architect(object):
    ...
    def _backward_step(self, val_data):
        ...
        loss = self.model.optimize_arch_params()
        ...
        if self.args["train"]["prior"] == True:
            if self.epoch < 10:
                ...
                for x in arch_params[2:]:
                    ...
                    reg = reg + torch.dist(
                        torch.nn.functional.softmax(x, dim=-1), mask, 1)
            elif (self.epoch >= 30) and (self.epoch < 40):
                for x in arch_params[3:]:
                    mask = torch.zeros([x.shape[-1]], device=x.device)
                    mask[-1] = 1
                    reg = reg + torch.dist(
                        torch.nn.functional.softmax(x, dim=-1), mask, 1)
            elif (self.epoch >= 40) and (self.epoch < 50):
                for x in arch_params[4:]:
                    mask = torch.zeros([x.shape[-1]], device=x.device)
                    mask[-1] = 1
                    reg = reg + torch.dist(
                        torch.nn.functional.softmax(x, dim=-1), mask, 1)
            elif (self.epoch >= 50) and (self.epoch < 60):
                for x in arch_params[5:]:
                    mask = torch.zeros([x.shape[-1]], device=x.device)
                    mask[-1] = 1
                    reg = reg + torch.dist(
                        torch.nn.functional.softmax(x, dim=-1), mask, 1)
