...
def main():
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, '*')))):
        ...
        img_lq = torch.from_numpy(img_lq).float().unsqueeze(0).to(
            device)  # CHW-RGB to NCHW-RGB
        ...
        with torch.no_grad():
            ...
            img_lq = torch.cat([img_lq, torch.flip(img_lq, [2])],
                               2)[:, :, :h_old + h_pad, :]
        ...
        if output.ndim == 3:
            output = np.transpose(output[[2, 1, 0], :, :],
                                  (1, 2, 0))  # CHW-RGB to HCW-BGR
        output = (output * 255.0).round().astype(np.uint8)  # float32 to uint8
        cv2.imwrite(f'{save_dir}/{imgname}_SwinIR.png', output)

        # evaluate psnr/ssim/psnr_b
        if img_gt is not None:
            img_gt = (img_gt * 255.0).round().astype(
                np.uint8)  # float32 to uint8
            ...
            print('Testing {:d} {:20s} - PSNR: {:.2f} dB; SSIM: {:.4f}; '
                  ...
                  'PSNR_B: {:.2f} dB.'.format(idx, imgname, psnr, ssim, psnr_y,
                                              ssim_y, psnr_b))
        else:
    ...
    if img_gt is not None:
        ...
        if img_gt.ndim == 3:
            ...
            ave_ssim_y = sum(test_results['ssim_y']) / len(
                test_results['ssim_y'])
            print('-- Average PSNR_Y/SSIM_Y: {:.2f} dB; {:.4f}'.format(
                ave_psnr_y, ave_ssim_y))
        if args.task in ['jpeg_car']:
            ave_psnr_b = sum(test_results['psnr_b']) / len(
                test_results['psnr_b'])
...
def get_image_pair(args, path):
    ...
    if args.task in ['classical_sr', 'lightweight_sr']:
        ...
        img_lq = cv2.imread(f'{args.folder_lq}/{imgname}x{args.scale}{imgext}',
