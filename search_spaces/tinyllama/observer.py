...
def get_block_size(
    input_shape: Tuple[int, ...], granularity: str, axis:int=-1
) -> Tuple[int, ...]:
    ...
    if granularity == 'tensor':
        return input_shape
    elif granularity == 'axis':
        block_size = list(input_shape)
        block_size[axis] = 1 ## axis = 1 for activation
...
class AffineQuantizedMinMaxObserver(AffineQuantizedObserverBase):
    def forward(self, input: torch.Tensor):
        ...
        block_size = get_block_size(input_detached.shape, self.granularity, self.axis) ### axis = 1 or ???
