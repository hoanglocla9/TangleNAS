...
from model_base_search import LlamaForCausalLM as FlexibleLlamaForCausualLM
from datasets import load_dataset, Dataset
import evaluate
from trainer import SupernetTrainer, SupernetTrainngArguments
...
@dataclass
class CustomTrainingArguments(SupernetTrainngArguments):
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    model.to('cuda')
    if config.mixop == 'gdas':
        model.sampler.set_taus(0.1, 10)  # Required only for GDAS
        model.sampler.set_total_epochs(100)  # Required only for GDAS
...
def local_dataset(dataset_name):
    ...
    elif os.path.exists(os.path.join(dataset_name, "data")) and "alpaca" in dataset_name:
        data_files = {"train":"train-00000-of-00001-a09b74b3ef9c3b56.parquet"}
        full_dataset = load_dataset("parquet", data_dir=os.path.join(dataset_name, "data"), data_files=data_files)["train"]
...
def train():
    ...
    tmp_training_args = CustomTrainingArguments()
    training_args.learning_rate = tmp_training_args.learning_rate
    training_args.adam_beta1 = tmp_training_args.adam_beta1
    training_args.adam_beta2 = tmp_training_args.adam_beta2
    training_args.weight_decay = tmp_training_args.weight_decay
    training_args.lr_scheduler_type = tmp_training_args.lr_scheduler_type
