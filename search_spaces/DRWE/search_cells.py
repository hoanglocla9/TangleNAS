...
import sys
from .cell_operations import OPS, ReLUConvBNSubSample
...
class NAS201SearchCell(nn.Module):
  ...
  def __init__(self, C_in, C_out, stride, max_nodes, op_names, affine=False, track_running_stats=True):
    ...
    #    self._ops[primitive] = ReLUConvBNSubSample(self._ops['nor_conv_3x3'], 1)
    #  else:
    ...
    for i in range(1, max_nodes):
      for j in range(i):
        ...
        if j == 0:
          ...
          _ops = torch.nn.ModuleDict()
          for primitive in op_names:
            if primitive == 'nor_conv_1x1':
              _ops[primitive] = ReLUConvBNSubSample(_ops['nor_conv_3x3'], 1)
            else:
              _ops[primitive] =  OPS[primitive](C_in , C_out, stride, affine, track_running_stats)
        else:
          ...
          for primitive in op_names:
            if primitive == 'nor_conv_1x1':
              ...
              _ops[primitive] =  OPS[primitive](C_in , C_out,      1, affine, track_running_stats)
        self.edges[ node_str ] = nn.ModuleList( list(_ops.values()) )
