...
import sys
# sys.path.insert(0, '../')
sys.path.insert(0, '/Users/blackheart/Work/re_saga/DrNAS_WE')
import time
import glob
import numpy as np
import torch
...
parser.add_argument('--nesterov', type=bool, default=True, help='partial channel parameter')
#### regularization
...
args.save = '/work/dlclarge1/sukthank-transformer_search/reproduce_oneshot/DrNAS/201-space/experiments/nasbench201/{}-search-{}-{}-{}'.format(
...
def print_model_summary(model):
    new_model_stats = ModelSummary(model, "New Model")
...
def main():
    ...
    #     api = API('/work/dlclarge1/sukthank-transformer_search/reproduce_oneshot/DrNAS/201-space/NAS-Bench-201-v1_0-e61699.pth')
    ...
    if args.method == 'gdas' or args.method == 'snas':
        ...
        model = TinyNetwork(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes,
                            criterion=criterion, search_space=NAS_BENCH_201, k=args.k, species='softmax', track_running_stats=False)
    # model = model.cuda()
    ...
    if args.dataset == 'cifar10':
        ...
        train_data = ImageNet16(root=os.path.join("/work/dlclarge1/sukthank-transformer_search/fresh_install/OneShotNASwithWE/data",'ImageNet16'), train=True, transform=train_transform, use_num_of_class_only=120)
