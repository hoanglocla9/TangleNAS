...
class _LRScheduler(object):
    ...
    def __repr__(self):
        return "{name}(warmup={warmup_epochs}, max-epoch={max_epochs}, current::epoch={current_epoch}, iter={current_iter:.2f}".format(
            name=self.__class__.__name__, **self.__dict__) + ", {:})".format(
                self.extra_repr())
    ...
    def update(self, cur_epoch, cur_iter):
        if cur_epoch is not None:
            assert (isinstance(cur_epoch, int) and cur_epoch >= 0
                    ), "invalid cur-epoch : {:}".format(cur_epoch)
            self.current_epoch = cur_epoch
        if cur_iter is not None:
            assert (isinstance(cur_iter, float) and
                    cur_iter >= 0), "invalid cur-iter : {:}".format(cur_iter)
    ...
    def get_lr(self):
        ...
        for base_lr in self.base_lrs:
            if self.current_epoch >= self.warmup_epochs:
                ...
                assert last_epoch >= 0, "invalid last_epoch : {:}".format(
                    last_epoch)
                ratio = ((self.max_LR - self.min_LR) * last_epoch /
                         self.max_epochs / self.max_LR)
                lr = base_lr * (1 - ratio)
            else:
                lr = (self.current_epoch / self.warmup_epochs +
                      self.current_iter / self.warmup_epochs) * base_lr
...
def get_optimizer(parameters, config):
    assert (
        hasattr(config, "optim") and hasattr(config, "criterion")
