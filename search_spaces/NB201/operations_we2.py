...
class SubConv(nn.Module):
  ...
  def forward(self, x):
    new_padding = int((self.kernel_size-1)/2) # Assumes stride = 1, dilation = 1
    x = F.conv2d(x,
                weight=self.op.weight[:,:,1:(self.kernel_size+1),1:(self.kernel_size+1)],
                bias=self.op.bias,
...
class ReLUConvBNMixture(nn.Module):
    def __init__(self, op, kernel_size_list, max_kernel_size):
        super(ReLUConvBNMixture, self).__init__()
        self.op = op
        self.kernel_list = kernel_size_list
        self.kernel_max = max_kernel_size
