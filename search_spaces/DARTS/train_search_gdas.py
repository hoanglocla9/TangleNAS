...
from model_search import DARTSSearchSpace
...
parser.add_argument('--k',
                    type=int,
                    default=6,
                    help='init partial channel parameter')
...
parser.add_argument(
    ...
    default=1e-3,
    help=
    'scaling factor of the regularization term, default value is proper for l2, for kl you might adjust reg_scale to match l2'
)
'''
...
parser.add_argument('--optimizer_type', type=str, default="drnas", help='optimizer type')
parser.add_argument('--tau_max', default=10, type=int, help='Tau max')
parser.add_argument('--tau_min', default=0.1, type=int, help='Tau min')
'''
...
def main():
    ...
    model = DARTSSearchSpace(
        args.optimizer_type, args.init_channels, CIFAR_CLASSES, args.layers,
        criterion
    )  #DARTSSearchSpace(args.optimizer_type,args.init_channels, CIFAR_CLASSES, args.layers, criterion,args.reg_type,args.reg_scale)
    model = model.cuda()
    ...
    for epoch in range(args.epochs):
        ...
        tau_curr = torch.Tensor([tau_curr])
        ...
        valid_acc, valid_obj = infer(test_queue, model, criterion, tau_curr)
        tau_curr += tau_step
...
def infer(valid_queue, model, criterion, tau_curr):
    objs = utils.AverageMeter()
    top1 = utils.AverageMeter()
    top5 = utils.AverageMeter()
    model.eval()
