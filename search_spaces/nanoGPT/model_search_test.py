...
from toy_search_spaces.nanoGPT.mixed_operations_test.mixed_embedding_attn import MixedEmbeddingAttention
from toy_search_spaces.nanoGPT.mixed_operations_test.mixed_attn_head_embed import MixedAttnHeadEmbed,  MixedAttnEmbd, MixedAttnHead
from toy_search_spaces.nanoGPT.mixed_operations_test.mixed_embedding import MixedEmbeddingV2
from toy_search_spaces.nanoGPT.mixed_operations_test.mixed_linear_head import MixedLinearHeadV2
from toy_search_spaces.nanoGPT.mixed_operations_test.mixed_linear_emb import MixedLinearV2Emb, MixedLinear
from toy_search_spaces.nanoGPT.mixed_operations_test.mixed_linear_emb_mlp import MixedLinearV2
from toy_search_spaces.nanoGPT.mixed_operations_test.mixed_layer_norm import MixedLayerNormV2
...
class Block(nn.Module):
    ...
    def forward(self, x, i, arch_params=None):
        ln1 = self.mixop.forward(x, 
                        arch_params["embed_dim"], self.ln1_list)
        print("in block after ln", torch.sum(ln1))
        x = x + self.attn(ln1, i, arch_params=arch_params)
        print("in block after attn", torch.sum(x))
        ln2 = self.mixop.forward(x,
                        arch_params["embed_dim"], self.ln2_list)
        print("In block after ln2", torch.sum(x))
        x = x + self.mlp(ln2, i, arch_params=arch_params)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        arch_parameters = [torch.nn.functional.softmax(a,dim=-1) for a in self.get_arch_parameters()]
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load("/gpfs/bwfor/work/ws/fr_rs1131-tanglenas/TangleNAS-dev/gpt_checkpoint.pth"),strict=False)
    model.eval()
