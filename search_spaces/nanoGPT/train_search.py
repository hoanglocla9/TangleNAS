...
# Loading values onto variable so that the configurator does not crash
...
out_dir = f"output_tinystories/out_search_{optimizer}_{train_portion}_{seed}_{max_iters}_{time_str}"
...
# optimizer
model_optimizer = model.configure_optimizers(
...
unwrapped_model = model.module if ddp else model  # unwrap DDP container if needed
...
unwrapped_model.sampler.set_total_epochs(100)  # Required only for GDAS
unwrapped_model.sampler.before_epoch()

arch_config_trajectory = {}
arch_traj_path_file = os.path.join(out_dir, "arch_traj.pkl")
X_arch, Y_arch = get_batch(
    "train", train_data=train_data_arch, val_data=None
)  # fetch the very first batch for arch
X_train, Y_train = get_batch(
    "train", train_data=train_data_weights, val_data=None
)  # fetch the very first batch for weights
t0 = time.time()
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses["val"] < best_val_loss or always_save_checkpoint:
            ...
            if iter_num > 0:
                ...
                arch_config_trajectory[str(iter_num)] = unwrapped_model.get_best_config()
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        if iter_num % log_interval == 0 and master_process:
            for p in unwrapped_model.get_arch_parameters():
                if p.grad is None:
                    print("Something is wrong")
    # grad clipping
    if grad_clip > 0:
        scaler.unscale_(architect.optimizer)
        torch.nn.utils.clip_grad_norm_(unwrapped_model.get_arch_parameters(), 5.0)
    ...
    if grad_clip != 0.0:
        ...
        torch.nn.utils.clip_grad_norm_(unwrapped_model.get_model_parameters(), grad_clip)
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if local_iter_num >= 5:  # let the training loop settle a bit
            mfu = unwrapped_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt)
