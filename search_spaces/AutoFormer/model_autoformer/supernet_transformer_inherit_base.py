...
#model =  Vision_TransformerSuper(img_size=32, patch_size=2, num_classes=10,change_qkv=True)
#config = {"embed_dim":[240]*14,
# "mlp_ratio": [4]*14,
# "layer_num" : 14,
# "num_heads" : [4]*14
# }
#model =  Vision_TransformerSuper(img_size=224, patch_size=16, num_classes=10, change_qkv=False,embed_dim=240,mlp_ratio=4,num_heads=3,depth=14,relative_position=True,scale=False)
#config = {"embed_dim":[192,216,240],"mlp_ratio": [3.5,4],"layer_num" : [12,13,14],"num_heads" : [3,4]}
#model =  Vision_TransformerSuper(img_size=32, patch_size=2, num_classes=10, change_qkv=False,embed_dim=240,mlp_ratio=4,num_heads=3,depth=14,relative_position=True,scale=False).cuda()

#ckpt = torch.load("/work/dlclarge2/sukthank-naslib_one_shot/OneShotNASwithWE/AutoFormerWE/output_cifar10_darts_df_final_with_prior/checkpoint_250.pth")
#temp_dict = {}
...
#import pickle
#import pandas as pd
#with open("/work/dlclarge2/sukthank-naslib_one_shot/OneShotNASwithWE/AutoFormerWE/output_cifar10_darts_df_final_with_prior/df_archs.pkl","rb") as f:
#     df = pickle.load(f)
...
#        print(correct/total)
#out = model(input)
'''model =  Vision_TransformerSuper(img_size=224, patch_size=16, num_classes=1000, change_qkv=True).cuda()
...
import requests
#feature_extractor = ViTFeatureExtractor.from_pretrained("google/vit-base-patch16-224-in21k").cuda()
from datasets import load_dataset

# load cifar10 (only small portion for demonstration purposes) 
train_ds, test_ds = load_dataset('cifar10', split=['train[:5000]', 'test[:]'])
#feature_extractor = ViTFeatureExtractor.from_pretrained('edumunozsala/vit_base-224-in21k-ft-cifar100')#.cuda()
model = ViTForImageClassification.from_pretrained('edumunozsala/vit_base-224-in21k-ft-cifar100').cuda()
...
with torch.no_grad():
    ...
    for data in testloader:
        images, labels = data
        # calculate outputs by running images through the network
        #inputs = feature_extractor(images=images)compute_loss
        #print(data)
