...
from search_spaces.AutoFormer.model_one_shot.module.linear_super import LinearSuper, LinearSubRatioEmb, LinearSubRatioMixture
from search_spaces.AutoFormer.model_one_shot.module.layernorm_super import LayerNormSuper, LayerNormSub, LayerNormMixture
from search_spaces.AutoFormer.model_one_shot.module.multihead_super import AttentionSuper
from search_spaces.AutoFormer.model_one_shot.module.embedding_super import PatchembedSuper, PatchembedSub, PatchembedMixture
from search_spaces.AutoFormer.model_one_shot.utils import trunc_normal_
from search_spaces.AutoFormer.model_one_shot.utils import DropPath
import itertools
import numpy as np
...
class Vision_TransformerSuper(nn.Module):
    ...
    def forward_features(self, x: torch.Tensor,
        ...
        #if self.gp:
        #if not self.use_we_v2:
        #print("Number of zeros", torch.sum(x==0))
        #assert torch.sum(out[:,216:]) == 0
        #else:
        #x = torch.mean(x[:, :, :][:, 1:], dim=1)
        #return x
        #print("Here")
        #print(torch.sum(x[:,:216]==out[:,:216]))
        #assert torch.allclose(x[:,:216],out[:,:216],atol=1e-3)
        if self.gp:
