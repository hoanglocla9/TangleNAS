...
from train.utils import distill, configure_optimizer, configure_scheduler
import logging

TrainingMetrics = namedtuple("TrainingMetrics", "loss acc_top1 acc_top5")


class OneShotSearchProgressive:
    ...
    def search(self, epochs):
        ...
        for i, current_epochs in enumerate(train_epochs):
            ...
            if not i == len(train_epochs) - 1:
                network.pruning(num_keeps[i + 1])
                network.wider(ks[i + 1])
                optimizer = configure_optimizer(self.model_optimizer,
                                                self.base_optimizer)
                scheduler = configure_scheduler(self.scheduler,
                                                self.base_scheduler)
                self.logger.log(
                    'pruning finish, {:.2f}% ops left per edge'.format(
                        num_keeps[i + 1]))
                self.logger.log(
                    'network wider finish, current pc parameter {:.2f}%'.
                    format(ks[i + 1]))

    def search_func(self, train_loader, valid_loader, network, criterion,
                    w_scheduler, w_optimizer, architect, alphas, epoch_str,
                    print_freq, logger, epoch):
