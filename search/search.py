...
class OneShotSearch:
    ...
    def _log_wandb(self, epoch):
        ...
        state = {
            ...
            "queried_train_acc": self.current_train_acc,
            "queried_test_acc": self.current_test_acc,
        }
    ...
    def _save_checkpoint(self, epoch, is_best):
        ...
        if self.save_every_epoch:
            save_path = save_checkpoint(
                state=state,
                filename=self.logger.path('model', epoch=epoch),
                logger=self.logger,
            )
            save_path = save_checkpoint(
            state=state,
    ...
    def search(self, epochs):
        if self.use_data_parallel == True:
            network, criterion = self._load_onto_data_parallel(
                self.search_model, self.criterion)
        else:
            network, criterion = self.search_model.to(
                self.device), self.criterion.to(self.device)
        ...
        for epoch in range(self.start_epoch, epochs):
            ...
            alphas = network.sampler.sample_epoch(
                network.arch_parameters(), sample_subset=self.sample_subset)
            ...
            valid_metrics = self.valid_func(
                val_loader, self.search_model, self.criterion, alphas)
            ...
            self.logger.log(
                "<<<--->>> The {:}-th epoch : {:}".format(epoch_str, self.genotypes[epoch]))

            if self.search_model.api is not None:
                if self.search_model.type == "toy":
                    result = self.search_model.query()
                    print(" Arch train {:.2f}% test {:.2f}%".format(
                        result["train_acc"], result["test_acc"]))
                    self.current_test_acc = result["test_acc"]
                    self.current_train_acc = result["train_acc"]
                else:
                    result = self.search_model.query()
                    logging.info('{:}'.format(result))
                    cifar10_train, cifar10_test, cifar100_train, cifar100_valid, \
                    cifar100_test, imagenet16_train, imagenet16_valid, imagenet16_test = distill(
                        result)
                    self.logger.log("cifar10 train {:.2f}% test {:.2f}%".format(
                    ...
                    self.current_test_acc = results['benchmark_cifar10_test']
                    self.current_train_acc = results['benchmark_cifar10_train']
    ...
    def search_func(self, train_loader, valid_loader, network, criterion,
        ...
        for step, (base_inputs, base_targets) in enumerate(train_loader):
            ...
            if architect is not None:
                ...
                arch_loss, arch_logits = architect.step(
            ...
            alphas = network.sampler.sample_epoch(
                network.arch_parameters(), sample_subset=self.sample_subset)
            _, logits = network(base_inputs, alphas=None)
        ...
        base_metrics = TrainingMetrics(
            base_losses.avg, base_top1.avg, base_top5.avg)
        arch_metrics = TrainingMetrics(
            arch_losses.avg, arch_top1.avg, arch_top5.avg)
