...
def benchmark_dataloader(args, kwargs):
    ...
    test_data = datasets.FashionMNIST(
        args.data, train=False, download=True,
        transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.2859,), (0.3530,)),
        ]))
...
def spos_search_dataloader(batch_size=64, train_portion=0.5):
    train_data = datasets.FashionMNIST(
    ...
    num_train_examples = len(train_data)
    indices = list(range(num_train_examples))
    split = int(np.floor(train_portion * num_train_examples))
