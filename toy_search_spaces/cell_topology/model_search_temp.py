...
class ToyCellSearchSpace(SearchNetworkBase):
    ...
    def mutate(self, arch, m_prob):
        k = sum(1 for i in range(self._nodes) for n in range(1 + i))
        num_ops = len(PRIMITIVES)
        arch_params_after_mutation = [torch.zeros_like(a) for a in arch]
        #print(arch_params_after_mutation[0].shape)
        #print(arch_params_after_mutation[1].shape)
        for i,alpha in enumerate(arch):
            for j in range(3):
                random_s = random.random()
                if random_s < m_prob:
                    random_op = random.randint(0, num_ops-1)
                    arch_params_after_mutation[i][j,random_op] = 1
                else:
                    arch_params_after_mutation[i][j] = alpha[j]
        return arch_params_after_mutation
    
    def crossover(self, arch1, arch2):
        k = sum(1 for i in range(self._nodes) for n in range(1 + i))
        num_ops = len(PRIMITIVES)
        arch_params_after_mutation = [torch.zeros_like(a) for a in arch1]   
        for i,alpha in enumerate(arch1):
            for j in range(3):
                random_s = random.random()
                if random_s < 0.5:
                    arch_params_after_mutation[i][j] = alpha[j]
                else:
                    arch_params_after_mutation[i][j] = arch2[i][j]
        return arch_params_after_mutation    
