...
do
    ...
    do
        ...
        do
            ...
            exp_name="toy_cell-${optimizer}-${dataset}-${ent}${we_version}"
            echo Submitting job $exp_name

            sbatch -J $exp_name job_scripts/job_experiment_ws.sh $dataset $optimizer "toy_cell" $ent $we_v2_flag $submission
        done
    done
done
